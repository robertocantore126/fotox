...
## M13-T01 (finish), T02, T04 — Models manager, Select Subject, Remove Background, Object Selection  (Claude, 2026-09-26)
- Done: AI jobs in `engine/m13.rs` (recipe R2: working image read on the engine thread, model on a worker, progress, **Esc or `ai:cancel` cancels**, document busy meanwhile); models cached once per process (`ai::model`). Preferences ▸ AI Models & ComfyUI (`dlg:prefs-ai`, `ui/js/native/prefs.js`): runtime path, models folder, per-model size / licence / Download (with progress) / Delete; the engine adds `_ai` to the preferences message. Select ▸ Subject (`ai:subject`, also a button on the Quick Selection and Object Selection bars and the Properties quick action) and Remove Background (`ai:remove-bg`, `Command::MaskFromModel`, one step). `SelectOp::Model(ModelMask)` (D-093): BiRefNet's 1024² mask stretched over the canvas, uniform tiles skipped, then M9-T05's guided filter in the edge band (radius ≈ 1.5 mask cells) — `fx-ops/src/select/model.rs`. Object Selection tool (`tools/object_select.rs`): Rectangle / Lasso (box + centre point), click = point prompt, Shift / Alt / option-bar Mode; EfficientSAM's embedding cached per document content generation. Without the runtime or a model the item says what is missing and where to get it (D-092). DirectML only when the runtime has it (D-095, a real crash found on Windows).
- Measured on this machine (CPU, ComfyUI's onnxruntime 1.29 as `FOTOX_ORT_DYLIB`): `fx-ai/tests/real_models.rs` BiRefNet IoU 1.000 on a synthetic disc (8–11 s), EfficientSAM box IoU 0.990 (encoder 1.7 s, decoder 64 ms); `fx-engine/tests/ai_flow.rs` through a running engine: Select Subject IoU 0.992 (12 s), Object Selection IoU 0.992, 2.1 s first / **1.2 s with the cached embedding (S39 ≤ 1.5 s met on CPU)**. In the app on a 3000 × 2000 PNG: Select Subject IoU 0.995 (thumbnail of the saved selection), Remove Background → one "Remove Background" step. The three real-model tests are `#[ignore]`d.
- Skipped: M13-T03 (Sky) and T05 (Neural Filters) by Rob's choice; Object Finder (hover highlight); Select Subject inside Select and Mask; shipping `onnxruntime.dll` (xtask / installer); DirectML untested (no DirectML build here); S38 on a 6000 × 4000 portrait with GPU; a real photo IoU fixture.
- FAST: the working image is read on the engine thread (bounded by 1024², mips served inline); one AI job at a time (a second one is refused with a toast); Remove Background refuses a layer that already has a mask (Photoshop intersects); the embedding cache keeps one document.
- VERIFY: Photoshop's Select Subject edge quality and its refinement radius.
- Try it: Edit ▸ Preferences ▸ AI Models & ComfyUI… ▸ Download; then Select ▸ Subject, or W ▸ Object Selection Tool and drag around an object; Properties ▸ Quick Actions ▸ Remove background.

## M13-T06 — Generative Fill and Generative Expand (ComfyUI bridge)  (Claude, 2026-09-26) — **written, not tested; parked by Rob ("niente generative fill per ora")**
- Done: `fx-ai/src/comfy.rs` (ping `/system_stats`, checkpoints from `/object_info`, multipart `/upload/image`, `/prompt`, polling `/history`, `/view`, `/interrupt` on cancel; every call has a timeout; bundled API workflows `fx-ai/assets/comfy/{inpaint,outpaint}.json` with `{{placeholders}}`: VAEEncodeForInpaint + RepeatLatentBatch → N variations). Engine: `ai:generative-fill {prompt}` reads the selection's bounds + 25 % context at ≤ 1024 long side (multiples of 8), alpha 0 where new content goes (LoadImage's mask); Crop tool Fill: Generative Expand (+ Prompt) sends the area outside the old canvas after the crop grows it. Results: `Command::GenerativeLayer` (D-094). Settings in Preferences ▸ AI: address (auto 127.0.0.1:8000 then :8188), checkpoint (auto: an inpainting, else an XL one), negative prompt, steps, CFG, variations, custom workflow files, Test connection (`comfy:test`). Edit ▸ Generative Fill… opens the prompt (`ui/js/native/generative.js`).
- Checked: only the unreachable case, in the app — a clear error in ~2 s, no hang. Template filling and PNG round trip have unit tests.
- Skipped: a real run through ComfyUI (S40), the mocked-ComfyUI test, Properties' variation switcher (toggle the layers' eyes), websocket progress (it is guessed from elapsed time).
- FAST: Expand sends the whole new canvas (a thin strip on a big image comes back soft); the generated alpha is ignored; the variations are stored in the command as raw RGBA8 (~3 × 4 MB per step).
- VERIFY: Photoshop's context margin around the selection; the grow-mask values in the workflows.
- Try it (when ComfyUI runs): select an area, Edit ▸ Generative Fill…, type a prompt, Generate.

## M13-T07 — Acceptance: deferred to HARDEN (S38–S40 on the reference machine, Rob's comparison with Photoshop).
