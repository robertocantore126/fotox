...
//! * Rotation is not supported yet (M6); keep every function rotation-free.
...
impl ViewTransform {
	...
	pub fn doc_to_screen(&self, viewport: ViewportSize, x: f64, y: f64) -> (f64, f64) {
		(
			(x - self.center_x) * self.zoom + viewport.width as f64 / 2.0,
			(y - self.center_y) * self.zoom + viewport.height as f64 / 2.0,
		)
	}

	pub fn screen_to_doc(&self, viewport: ViewportSize, sx: f64, sy: f64) -> (f64, f64) {
		(
			(sx - viewport.width as f64 / 2.0) / self.zoom + self.center_x,
			(sy - viewport.height as f64 / 2.0) / self.zoom + self.center_y,
		)
	}

	/// Pan by a screen-space delta (hand tool drag).
	...
	pub fn zoom_at(&mut self, viewport: ViewportSize, sx: f64, sy: f64, new_zoom: f64) {
		...
		self.center_x = dx - (sx - viewport.width as f64 / 2.0) / self.zoom;
		self.center_y = dy - (sy - viewport.height as f64 / 2.0) / self.zoom;
	}
	...
	pub fn visible_doc_rect(&self, viewport: ViewportSize, doc_w: u32, doc_h: u32) -> Option<(f64, f64, f64, f64)> {
		let (ax, ay) = self.screen_to_doc(viewport, 0.0, 0.0);
		let (bx, by) = self.screen_to_doc(viewport, viewport.width as f64, viewport.height as f64);
		let x0 = ax.max(0.0);
		let y0 = ay.max(0.0);
		let x1 = bx.min(doc_w as f64);
		let y1 = by.min(doc_h as f64);
