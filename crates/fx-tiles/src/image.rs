...
impl TiledImage {
	...
	pub fn new(width: u32, height: u32, format: PixelFormat) -> Self {
		...
		loop {
			// Level 0 starts clean; an all-empty level 0 means all-empty mips, so they are clean too.
			levels.push(TileGrid::new(tiles_for(w), tiles_for(h), false));
		...
		Self { width, height, format, levels }
	}
	...
	/// Store a recomputed mip tile (level >= 1) and clear its dirty flag.
	pub fn set_derived_slot(&mut self, level: usize, tx: u32, ty: u32, slot: TileSlot) {
		assert!(level >= 1, "level 0 is authoritative, use set_slot");
	...
	/// True if a mip tile must be recomputed before use. Level 0 is never dirty.
