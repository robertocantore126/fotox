//! Blend modes and their maths (D-047: the pure functions live here so the
//! brush engine in `fx-ops` can use them without a GPU dependency;
//! `fx-render::blend` re-exports them). f64, straight colour. Formulas:
//! docs/BLEND_MODES.md. Keep this obviously correct, not fast.
...
/// the functions below (the CPU reference; the GPU shaders in `fx-render`'s
/// `gpu/composite.wgsl` are tested against them). Do not invent formulas:
/// match that document.
